...
func main() {
	...
	saved, err := db.Users().Save(ctx, gen.User{
		Id:        id,
		Username:  "Lucene Text target (table, minScore)",
		Email:     fmt.Sprintf("lucene-table-minscore-%d@example.com", time.Now().UnixNano()),
		IsActive:  true,
		CreatedAt: now,
		UpdatedAt: now,
	})
	if err != nil {
		log.Fatalf("FAILED: ailed to seed user: %v", err)
	}
	if saved.Id == "" {
	...
	for attempt := 1; attempt <= 6; attempt++ {
		users, err := db.Users().
			Where(sdk.Search(searchText, searchMinScore)).
			List(ctx)
		if err != nil {
	...
	log.Println("found seeded user in table search with minScore!")
}
