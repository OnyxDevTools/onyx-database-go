...
type UserRepository interface {
	...
	FirstOrNil(ctx context.Context) (*User, error)
...
func (c UsersClient) AsMaps() UsersMapClient {
	return UsersMapClient{core: c.core, q: c.q, timeout: c.timeout, hook: c.hook}
}
...
func (c UsersClient) FirstOrNull(ctx context.Context) (*User, error) {
	limited := c.Limit(1)
	ctx, done := withContextAndHook(ctx, limited.timeout, limited.hook, "first_or_null", Tables.User)
	res := onyx.List(ctx, limited.q)
	var out []User
	if err := res.Decode(&out); err != nil {
		err = fmt.Errorf("failed to decode user first_or_null: %w", err)
		done(err)
		return nil, err
	}
	done(nil)
...
func (c UsersClient) FirstOrNil(ctx context.Context) (*User, error) { return c.FirstOrNull(ctx) }
func (c UsersClient) One(ctx context.Context) (User, error) {
	limited := c.Limit(2)
	ctx, done := withContextAndHook(ctx, limited.timeout, limited.hook, "one", Tables.User)
	res := onyx.List(ctx, limited.q)
	var out []User
	if err := res.Decode(&out); err != nil {
		err = fmt.Errorf("failed to decode user one: %w", err)
		done(err)
		return User{}, err
	}
	done(nil)
	if len(out) == 0 {
		return User{}, fmt.Errorf("expected one user, got 0")
	}
	if len(out) > 1 {
		return User{}, fmt.Errorf("expected one user, got %d", len(out))
	}
	return out[0], nil
}
...
func (c UsersClient) Pages(ctx context.Context) *UsersPageIterator {
	return &UsersPageIterator{client: c, ctx: ctx}
}
...
func (c UsersMapClient) Pages(ctx context.Context) *UsersMapPageIterator {
	return &UsersMapPageIterator{client: c, ctx: ctx}
}
