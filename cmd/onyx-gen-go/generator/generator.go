...
func renderClient(schema onyx.Schema, pkg string) []byte {
	...
	for _, t := range schema.Tables {
		...
		fmt.Fprintf(&buf, "type %s struct { q onyx.Query }\n\n", queryName)
		fmt.Fprintf(&buf, "func (q %s) Where(cond onyx.Condition) %s { q.q = q.q.Where(cond); return q }\n", queryName, queryName)
		fmt.Fprintf(&buf, "func (q %s) And(cond onyx.Condition) %s { q.q = q.q.And(cond); return q }\n", queryName, queryName)
		fmt.Fprintf(&buf, "func (q %s) Or(cond onyx.Condition) %s { q.q = q.q.Or(cond); return q }\n", queryName, queryName)
		...
		fmt.Fprintf(&buf, "type %s struct { q onyx.Query; ctx context.Context }\n\n", deleteName)
		fmt.Fprintf(&buf, "func (d %s) Where(cond onyx.Condition) %s { d.q = d.q.Where(cond); return d }\n", deleteName, deleteName)
		fmt.Fprintf(&buf, "func (d %s) And(cond onyx.Condition) %s { d.q = d.q.And(cond); return d }\n", deleteName, deleteName)
		fmt.Fprintf(&buf, "func (d %s) Or(cond onyx.Condition) %s { d.q = d.q.Or(cond); return d }\n", deleteName, deleteName)
		...
		fmt.Fprintf(&buf, "func (c Client) Save%s(ctx context.Context, item %s, cascades ...onyx.CascadeSpec) (%s, error) {\n", typeName, typeName, typeName)
		...
		fmt.Fprintf(&buf, "type %s struct { core onyx.Client }\n\n", serviceName)
		...
		fmt.Fprintf(&buf, "type %s struct { core onyx.Client; spec onyx.CascadeSpec }\n\n", cascadeName)
		fmt.Fprintf(&buf, "func (s %s) Cascade(spec onyx.CascadeSpec) %s { return %s{core: s.core, spec: spec} }\n\n", serviceName, cascadeName, cascadeName)
